...
export default function CreateDish1() {
  ...
  useEffect(() => {
    async function fetchInitialData() {
      try {
        ...
        setDishData(catererData.dishesfor10_25 || []);
        setPackages(
          (catererData.dishesfor10_25 || []).map((dish) => ({
            ...dish,
  ...
  const handleDishChange = (packageIndex, dishIndex, event) => {
    ...
    if (event.target.name === "item" && selectedDish) {
      updatedPackages[packageIndex].items[dishIndex].id = selectedDish.id;
    }
    setPackages(updatedPackages);
  };
  ...
  const removeDish = (packageIndex, dishIndex) => {
    const updatedPackages = [...packages];
    updatedPackages[packageIndex].items.splice(dishIndex, 1);
    setPackages(updatedPackages);
  };
    ...
    try {
      ...
      if (newdishes.length > 0) {
        const res=await axios.patch(`https://www.caterersnearme.in/api/caterer/${catererId}`, {
          dishesfor10_25: [...dishData.map(el=>el.id), ...newdishes.map(el=>el.id)],
        });
      }
      ...
      <form className={styles.formContainer}>
        {packages.map((pkg, pkgIndex) => (
          <div key={pkgIndex} className={styles.package}>
            ...
            <h4>dishes for 10 to 25 people</h4>
