...
export default function CreateDish2() {
  ...
  useEffect(() => {
    async function fetchInitialData() {
      try {
        ...
        setDishData(catererData.dishesfor25_50 || []);
        setPackages(
          (catererData.dishesfor25_50 || []).map((dish) => ({
            ...dish,
    ...
    try {
      ...
      if (newdishes.length > 0) {
        const res=await axios.patch(`http://localhost:3000/api/caterer/${catererId}`, {
          dishesfor25_50: [...dishData.map(el=>el.id), ...newdishes.map(el=>el.id)],
        });
      }
